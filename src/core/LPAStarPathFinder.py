...
class LPAStarPathFinder:
    ...
    def find_path(self, goal: Tuple[float, float]) -> None:
        """ Entry point function which is responsible to rescan map, recalculate optimal path if necessary and update agent.
            First, it calls reset, after that it calls sensor's scan function, converts obstacles to its graph representation and compares them to the
    ...
    def __update_vertex(self, v: Tuple[int, int]) -> None:
        """ Updates the rhs-value of the vertex and reinserts it in priority queue with new key if necessary.
            The rhs-value of the vertex is updated according to the LPA* algorithm rhs-formula. You can find
            it in README of the repository.
    ...
    def compute_shortest_path(self) -> List[Tuple[int, int]]:
        """ Computes the shortest path using the advantages of LPA* algorithm. While the distance to the goal
            vertex (g-value) is not optimal and can be updated (g-value is different from rhs-value) we update
            the g-value of the vertex on the top of the priorirty queue and then we update its neighbours.
